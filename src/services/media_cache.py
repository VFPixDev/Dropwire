"""Telegram file_id cache used by inline Rich Messages."""
...
async def _get_or_upload(bot, database: Database, chat_id: int, item: MediaItem) -> CachedMedia | None:
    ...
    async with _cache_upload_slots:
        ...
        try:
            if item.type == "photo":
                message = await bot.send_photo(chat_id=chat_id, photo=media_url, disable_notification=True)
            ...
            logger.info("Telegram отклонил загрузку в медиакэш: %s", exc)
            return None

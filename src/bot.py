...
def build_dispatcher(bot: BotAdapter, application: ApplicationState) -> Dispatcher:
    ...
    async def on_cache_binding(message: Message) -> None:
        ...
        if str(status) not in {"administrator", "creator", "owner"} or can_post is False:
            await message.answer("Бот должен быть администратором канала-кэша.")
            return
        database = application.bot_data.get("database")
        ...
        pending.discard(user_id)
        await message.answer(f"Медиакэш подключён: <code>{channel.id}</code>.")
